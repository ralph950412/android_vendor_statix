...
PROD_OTA_PACKAGE_TARGET := $(PRODUCT_OUT)/$(STATIX_VERSION).zip
...
.PHONY: stx-prod
stx-prod: $(PROD_OTA_PACKAGE_TARGET)

ifneq ($(PREVIOUS_TARGET_FILES_PACKAGE),)

INCREMENTAL_OTA_PACKAGE_TARGET := $(PRODUCT_OUT)/incremental-$(STATIX_VERSION).zip
...
$(INCREMENTAL_OTA_PACKAGE_TARGET): $(SIGNED_TARGET_FILES_PACKAGE) \
		$(INTERNAL_OTATOOLS_FILES) $(OTA_FROM_TARGET_FILES)
	@echo "statix incremental production: $@"
	    $(OTA_FROM_TARGET_FILES) --verbose \
	    -p $(OUT_DIR)/host/linux-x86 \
	    -k $(KEY_CERT_PAIR) \
